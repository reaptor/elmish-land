...
let validate (projectDir: AbsoluteProjectDir) =
    eff {
        ...
        let includedFSharpFileInfo =
            fs.ReadAllLines(FsProjPath.fromProjectDir projectDir |> FsProjPath.asFilePath)
            ...
            |> Array.choose (fun (lineNr, line) ->
                ...
                if m.Success then
                    Some(
                        lineNr,
                        line,
                        m.Groups[1].Value |> FilePath.fromString |> FilePath.ensureRelativeTo projectDir
                    )
                else
                    None)
            |> Array.toList
        ...
        let actualPageFiles =
            fs.GetFilesRecursive(pagesDir, "Page.fs")
