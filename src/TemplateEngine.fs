...
let getSortedPageFiles (projectDir: ProjectDir) =
    Directory.GetFiles(ProjectDir.asString projectDir, "Page.fs", EnumerationOptions(RecurseSubdirectories = true))
...
let fileToRoute (projectDir: ProjectDir) (FilePath file) =
    let route =
        file[0 .. file.Length - 9]
        |> String.replace
            (ProjectDir.asFilePath projectDir
...
let getRouteData (projectDir: ProjectDir) =
