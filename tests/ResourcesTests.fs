...
let ``Routes_template with F# keyword route parameters`` () =
    let templateDataWithKeyword = {
        ...
        RootModule = "TestApp"
        ElmishLandAppProjectFullName = "ElmishLand.TestApp.App"
        Routes = [|
            {
                Name = "Home"
                RouteName = "HomeRoute"
...
module Routes =
    ...
    type HomeRoute = unit
    type ArbitraryPage_NewRoute = { New: string }

    [<RequireQualifiedAccess>]
    type Route =
        | Home of HomeRoute
        | ArbitraryPage_New of ArbitraryPage_NewRoute
        | NotFound
    ...
    module Route =
        ...
        let format =
            ...
            | Route.ArbitraryPage_New { New = ``new`` } -> Router.format("arbitrary-page", ``new``)
            | Route.NotFound -> "notFound"
        ...
        let parse (xs: string list) =
            ...
            match xs with
            | [ Query q ] -> Route.Home ()
            | [ arbitraryPage; ``new``; Query q ] when eq arbitraryPage "arbitrary-page" -> Route.ArbitraryPage_New { New = ``new``; }
            | other ->
                printfn "Route not found: '%A'" other
                Route.NotFound
